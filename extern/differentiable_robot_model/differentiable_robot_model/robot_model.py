...
class DifferentiableRobotModel(torch.nn.Module):
    ...
    def compute_forward_kinematics_all_links(
        ...
        return {
            link.name: (self._bodies[self._name_to_idx_map[link.name]].pose.translation(), self._bodies[self._name_to_idx_map[link.name]].pose.rotation())
            for link in self._bodies
        }
